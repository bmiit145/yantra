    ...
    <div class="o_dropdown_container o_filter_menu w-100 w-lg-auto h-100 px-3 mb-4 mb-lg-0 border-end">
        ...
        <span class="o-dropdown-item-2 dropdown-item o-navigable o_menu_item text-truncate activities"
            role="menuitemcheckbox" tabindex="0" title="" aria-checked="false"><span class="float-end checkmark"
                style="display:none;">✔</span>Open Opportunities</span>
                        ...
                        <div name="user_id" class="o_field_widget o_field_many2one_avatar_user d-inline-block">
                            <div class="d-flex align-items-center gap-1"
                                ...
                                @php
                                    ...
                                    if (!function_exists('generateColorFromEmail')) {
                                    ...
                                    if ($user) {
                                        $bgColor = generateColorFromEmail($user->email);
                                    } else {
                                        $bgColor = '#f0f0f0'; // Default color if user not found
                                    }
                                @endphp
...
function filterData(selectedTags) {
    ...
    $.ajax({
        url: '{{ route('crm.pipeline.filter.activities') }}',
        method: 'GET',
...
function generateColorFromEmail(email) {
    // Hash function to generate a numeric value from the email
    let hash = 0;
...
function generateActivityHtml(activities) {
    ...
    Object.entries(groupedActivities).forEach(([leadId, activities]) => {
        ...
        const userEmail = lead.get_pipeline_lead_title?.email || 'Unknown User';
        const bgColor = generateColorFromEmail(userEmail); // Generate color based on email
        ...
        html += `
            <tr class="o_data_row h-100">
                <td class="o_activity_record p-2 cursor-pointer">
                    <div>
                        <div name="user_id" class="o_field_widget o_field_many2one_avatar_user d-inline-block">
                            <div class="d-flex align-items-center gap-1" data-tooltip="${userEmail}">
                                ${generateUserAvatar(lead.get_pipeline_lead_title?.profile, userInitial, bgColor)}
                            </div>
                        </div>
                        <div class="flex-grow-1">
                ...
                ${generateActivityCells(activities, lead.pipeline_id, bgColor)}
            </tr>
        `;
    });
...
function groupByLead(activities) {
    return activities.reduce((acc, activity) => {
        const leadId = activity.pipeline_id;
...
function generateLeadInfo(lead) {
    return `
        <div class="d-flex justify-content-between">
            <div class="d-block text-truncate o_text_block o_text_bold ml-3">${lead.opportunity}</div>
            <div name="expected_revenue" class="o_field_widget o_field_empty o_field_monetary d-block text-truncate text-muted">
                <span>₹${lead.get_pipeline_lead_title ? lead.get_pipeline_lead_title.expected_revenue : '0.00'}</span>
            </div>
        </div>
        <div class="d-flex justify-content-between">
            <div class="d-block text-truncate text-muted o_text_block"></div>
...
$.ajax({
url: '{{ route('crm.pipeline.activity.custom.filter') }}',
method: 'POST',
data: data,
success: function (response) {
            ...
            $('#customer_filter_select').on('change', function () {
                ...
                switch (selectedValue) {
                    ...
                    case 'Source':
                        ...
                        break;
                    case 'Stage':
                        filterInput.append('<select name="stage" id="customer_filter_input_value" class="form-control">@foreach($CrmStages as $Stage)<option value="{{ $Stage->title }}">{{ $Stage->title }}</option>@endforeach</select>');
                        break;
                    case 'State':
...
<script>
   $(document).ready(function() {
    ...
    $('.dropdown-item').on('click', function() {
        ...
        $.ajax({
url: '{{ route('crm.pipeline.activity.input.filter') }}',
method: 'POST',
data: { 
...
function generateActivityHtml(activities) {
    ...
    Object.entries(groupedActivities).forEach(([leadId, activities]) => {
        const lead = activities[0]; // Assuming lead info is consistent
        const userEmail = lead.get_pipeline_lead_title.sales_person?.email || 'Unknown User';
        const bgColor = generateColorFromEmail(userEmail); // Generate color based on email
        const userInitial = getUserInitial(userEmail);
...
function generateActivityDetails(counts, activities) {
    ...
    details += `<div class="overflow-auto d-flex align-items-baseline ms-3 me-1 mt-2"><b>Overdue (${counts.overdue})</b></div>`;
    details += `<div class="o-mail-ActivityListPopoverItem d-flex flex-column border-bottom py-2">`;
    const overdueActivities = activities.filter(activity => new Date(activity.due_date) < new Date());

    // Check if there are no "today" activities
    if (counts.today === 0) {
